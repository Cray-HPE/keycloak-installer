...
class KeycloakClient(object):
    ...
    *Exception Handling*
    ...
    def __init__(self,
        ...
        if not issubclass(type(realm), types.StringTypes):
            raise TypeError("invalid realm")

        if not issubclass(type(client_id), types.StringTypes):
            raise TypeError("invalid client_id")

        if k8s_secret_name is not None:
            if not issubclass(type(k8s_secret_name), types.StringTypes):
        ...
        if k8s_secret_namespaces is not None:
            ...
            for ns in k8s_secret_namespaces:
                if not issubclass(type(ns), types.StringTypes):
                    raise TypeError(
    ...
    def create_role(self, role):
        ...
        if not issubclass(type(role), types.StringTypes):
...
def k8s_apply_secret(namespace, secret_name, secret_data, v1=None):
    ...
    secret_data_encoded = {k: base64.b64encode(v) for k, v in secret_data.items()}
